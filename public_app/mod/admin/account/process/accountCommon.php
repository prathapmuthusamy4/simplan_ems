...
class accountCommon extends adminProcess
{
    var $error_info;
    var $table_name;
    var $app_name;
    ...
    function makeSearchParam($param)
    {
        ...
        // 管理者区分
        if (!is_empty($param["f_admin_kbn"])) {
            $where["f_admin_kbn"] = $param["f_admin_kbn"];
        }
        return $where;
    }
    ...
    {
        $key = array("f_admin_id" => $param["f_admin_id"]);
        return $key;
    }
   ...
   function inputCheck($input, &$param)
    {
        ...
        // duplicate id
        if (is_empty(t_array_value("f_id", $errList))) {
            if (!$com->dup_id($this->table_name, $input["f_id"], $param["f_admin_id"])) {
                $errList["f_id"] = $this->getMessage(STATUS_COMMON, "C_0011");
            }
        }
        // duplicate mail
        if (is_empty(t_array_value("f_mailaddress", $errList)) && !is_empty($input["f_mailaddress"])) {
            if (!$com->dup_mail($this->table_name, $input["f_mailaddress"], $param["f_admin_id"])) {
                $msg = $this->getMessage(STATUS_COMMON, "C_0024");
                $errList['f_mailaddress'] = sprintf($msg, 'Mail');
            }
        }
        /********************
         * judge
